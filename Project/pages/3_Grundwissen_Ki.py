...
with st.spinner(text="Erstelle Text, bitte warten..."):
    ...
    st.write(f"- OpenAI Client vorhanden: {'✅' if openai_client else '❌'}")
    st.write(f"- API Key 1 vorhanden: {'✅' if api_key1 else '❌'}")
    st.write(f"- API Key 2 vorhanden: {'✅' if api_key2 else '❌'}")
    st.write(f"- Replicate Client vorhanden: {'✅' if replicate_client else '❌'}")
   
    # API-Aufruf an OpenAI Key 1
    try:
        st.write("🔄 **Versuche OpenAI Key 1...**")
        ...
        st.write("✅ **OpenAI Key 1 erfolgreich!**")
        
    except Exception as e1:
        st.write(f"❌ **OpenAI Key 1 Fehler:** {str(e1)}")
        
        # Versuche Key 2
        if api_key2:
            try:
                st.write("🔄 **Versuche OpenAI Key 2...**")
                antwort = openai_client.chat.completions.create(
                        model="gpt-3.5-turbo",
                        messages=[{"role": "user", "content": f"Beantworte die Frage nur auf Deutsch: {frage}"}]
                    )
                antwort_text = antwort.choices[0].message.content
                st.write("✅ **OpenAI Key 2 erfolgreich!**")
                
            except Exception as e2:
                st.write(f"❌ **OpenAI Key 2 Fehler:** {str(e2)}")
                
                # Fallback zu Replicate
                if replicate_client:
                    try:
                        st.write("🔄 **Versuche Replicate...**")
                        antwort_text = ""
                        ...
                        st.write(f"❌ **Replicate Fehler:** {str(e3)}")
                        antwort_text = "Die Antwort konnte nicht generiert werden."
                else:
                    st.write("❌ **Kein Replicate Client verfügbar**")
        else:
            st.write("⚠️ **Kein OpenAI Key 2 verfügbar - springe zu Replicate**")
            # Direkt zu Replicate wenn kein Key 2
            if replicate_client:
                try:
                    st.write("🔄 **Versuche Replicate...**")
                    antwort_text = ""
